...
class LoanManagement(models.Model):
    ...
    next_payment_date = fields.Date(string='Next Installment Date',
                                    compute='_compute_next_payment_date', readonly=False)
    ...
    @api.depends('first_installment_date')
    def _compute_next_payment_date(self):
        for record in self:
            if record.first_installment_date:
                next_month = record.first_installment_date + relativedelta(months=+1)
                next_payment_date = next_month.replace(day=record.first_installment_date.day)
